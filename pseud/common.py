...
internal_exceptions = tuple(
    name
    for name in dir(interfaces)
    if inspect.isclass(getattr(interfaces, name))
    and issubclass(getattr(interfaces, name), Exception)
)
...
def format_remote_traceback(traceback):
    ...
    return textwrap.dedent(
        ...
        """.format(
            pivot.join(str(traceback).splitlines())
        )
    )
...
class BaseRPC(object):
    def __init__(
        ...
        self.auth_backend = zope.component.getAdapter(
            self, IAuthenticationBackend, name=self.security_plugin
        )
        ...
        self.heartbeat_backend = zope.component.getAdapter(
            self, IHeartbeatBackend, name=heartbeat_plugin
        )
    ...
    def __getattr__(self, name, default=_marker):
        try:
            ...
            if not self.initialized:
                raise RuntimeError(
                    'You must connect or bind first' ' in order to call {!r}'.format(name)
                )
    ...
    async def on_socket_ready(self, response):
        ...
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(
                ...
                'meta: {} message: {}'.format(
                    (self.user_id.hex() if self.user_id is not None else user_id.hex()),
        ...
        return await self.dispatch(
            message_type, message, routing_id, user_id, message_uuid
        )

    async def dispatch(self, message_type, message, routing_id, user_id, message_uuid):
        if message_type == WORK:
            return await self._handle_work(message, routing_id, user_id, message_uuid)
        if message_type == OK:
        ...
        if message_type == HELLO:
            return self.auth_backend.handle_hello(
                user_id, routing_id, message_uuid, message
            )
    ...
    async def _handle_work_proxy(self, locator, args, kw, user_id, message_uuid):
