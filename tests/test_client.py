...
def test_client_creation():
    ...
    identity = __name__
    server_identity = 'echo'
    client = Client(identity, server_identity)
    assert client.server_identity == server_identity
...
class ClientTestCase(tornado.testing.AsyncTestCase):
    ...
    def make_one_client(self, identity, server_identity, io_loop=None):
        from pyzmq_rpc import Client
        client = Client(identity, server_identity, io_loop=io_loop)
        return client
    ...
    def test_client_method_wrapper(self):
        ...
        identity = __name__
        server_identity = 'echo'
        client = self.make_one_client(identity, server_identity,
    ...
    def test_job_executed(self):
        ...
        socket = self.make_one_server_socket(server_identity, endpoint)
        client = self.make_one_client(identity, server_identity,
                                      io_loop=self.io_loop)
        client.connect(endpoint)

        stream = zmqstream.ZMQStream(socket, io_loop=self.io_loop)
        result = yield client.please.do_that_job(1, 2, 3, b=4)
        ...
        # assert server_id == server_identity
